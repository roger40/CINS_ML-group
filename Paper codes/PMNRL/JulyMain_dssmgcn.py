...
def train(model, optimizer, epochs, adj_norm, criterion, weight_loss2_1, weight_loss2_2,
    ...
    for epoch in range(epochs):
        ...
        print('epoch:', epoch, 'Train Loss:', loss_total.item(), 'loss_dssm:', loss2_1_history[-1], 'loss_classify:', loss2_2_history[-1],
        ...
        if val_acc_history[-1] >= best_val_acc or val_loss_history[-1] <= best_val_loss:
            ...
            # if val_acc_history[-1] >= best_val_acc and val_loss_history[-1] <= best_val_loss:
                ...
                # np.save(result_path+'rep/'+dataset_name+'_cluster_best.npy', logits2_1.detach().cpu().numpy())
                np.save(result_path+'rep/'+dataset_name+'_node_best.npy', logits2_2.detach().cpu().numpy())
    ...
    plt.figure()
    plt.plot(range(1, len(loss2_1_history)+1), loss2_1_history, label='loss_dssm_history', color='red')
    plt.plot(range(1, len(loss2_2_history)+1), loss2_2_history, label='loss_classify_history', color='black')
...
if __name__ == '__main__':
    ...
    weight_loss2_2 = 1
    hidden_num = 32
    output_dim2_1 = 128   # 无监督聚类的嵌入维度
    output_dim2_2 = y_labels.shape[1]
    cluster_num = y_labels.shape[1]  # 聚类数目也可以暂定为实际类别数
    ...
    def main(num, dataset_name, adj, features, y_labels, train_mask, val_mask, test_mask, y_class,
             dropout, weight_decay, learning_rate, weight_loss2_1, weight_loss2_2, hidden_num, output_dim2_1, output_dim2_2, cluster_num):
        ...
        best_test_acc = train(model=mutiltask_model, optimizer=optimizer, epochs=epochs, adj_norm=adj_norm,
                              criterion=criterion, weight_loss2_1=weight_loss2_1, weight_loss2_2=weight_loss2_2,
                              feature_X=feature_X, cluster_num=cluster_num, train_y2_2=train_y2_2,
