...
def manhattan_distance(obs, centers):
    dis = (torch.abs(obs.unsqueeze(dim=1) - centers.unsqueeze(dim=0))).sum(dim=-1).squeeze()
    return dis
...
def _kmeans_batch(obs: torch.Tensor,
    ...
    while True:
        ...
        for i in range(k):
            ...
            # 719
            obs_in_cluster = obs.index_select(0, obs_id_in_cluster_i.nonzero(as_tuple=False).squeeze())
            # 719
